...
const GenderTable = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await axios.get('https://nuniali-51afdf69a4d2.herokuapp.com/genders', { params: query });
    ...
    const handleDelete = async () => {
        setLoading(true);
        try {
            await axios.delete(`https://nuniali-51afdf69a4d2.herokuapp.com/genders/${toBeDeletedId}`, {
                headers: {
    ...
    const handleCreateNew = () => {
        navigate('/admin/gender/create');
    };

    const handleEdit = (id) => {
        navigate(`/admin/gender/update/${id}`);
    };
