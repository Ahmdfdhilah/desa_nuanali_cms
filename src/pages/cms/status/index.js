...
const StatusTable = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await axios.get('https://nuniali-51afdf69a4d2.herokuapp.com/statuses', { params: query });
    ...
    const handleDelete = async () => {
        setLoading(true);
        try {
            await axios.delete(`https://nuniali-51afdf69a4d2.herokuapp.com/statuses/${toBeDeletedId}`, {
                headers: {
    ...
    const handleCreateNew = () => {
        navigate('/admin/status/create');
    };

    const handleEdit = (id) => {
        navigate(`/admin/status/update/${id}`);
    };
